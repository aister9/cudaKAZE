...
public:
	...
	void setTimerTitle ( const char* _name ) { memset(timerTitle, 0, sizeof(char)*255) ; memcpy(timerTitle, _name, strlen(_name)) ; }

	void setTimerName (UINT id, std::string &_name) { timerName[id] = _name ; }
	void setTimerName (UINT id, const char* _name) { timerName[id] = _name ;}
